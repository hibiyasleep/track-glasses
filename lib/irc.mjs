    ...
    const string = (() => {
      ...
      switch(command) {
        ...
        case 'PART':
          return log({
            objective: params,
            user: util.uid.name(name),
            userColor: util.uid.color(name),
            command,
            parentheses: ' '
          })

        case 'CLEARCHAT':
          const id = tags.user_id
          return log({
            objective: params,
            user: chalk.strikethrough(util.uid.name(id) ?? content),
            userColor: util.uid.color(tags?.color),
            parentheses: '--',
            command,
            verb: 'for',
        ...
        case 'PASS':
          return log({ command, params: params.slice(0, 10) + '*'.repeat(params.length - 10) })
